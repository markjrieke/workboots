...
From each set of nested predictions, we can summarize with a lower and upper bound of our prediction interval (this uses the [`quantile()`](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/quantile) function under the hood). 
...
Our XGBoost model is overfit to the training set --- in a more serious analysis we'd finalize tuning parameters prior to calling `predict_boots()`. 
